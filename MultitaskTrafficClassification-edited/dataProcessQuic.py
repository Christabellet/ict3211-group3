...
def loadData(dirPath, class_label, extractedFlows=0):
    ...
    for folder, subs, files in os.walk(dirPath):
        ...
        for file in files:
            ...
            with open(filename) as f:
                ...
                # Skip the first few packets in the file
                if Starting_point != 0:
                ...
                for subflow in range(extractedFlows):
                    ...
                    while numOfSamples < timestep:
                        ...
                        data = list(EntireFile[i])  # To clone the list, not referring to the same list

                        # shahbaz: To descretesize the length
    ...
    with open(r'dataList.txt', 'w') as fp:
        for item in datalist:
            fp.write("%s\n" % item)
    print('Done datalist')

    with open(r'labelList.txt', 'w') as fp:
        for item in labellist:
            fp.write("%s\n" % item)
    print('Done labellist')
...
if __name__ == "__main__":
    ...
    (data1, label1) = loadData(os.path.join(BaseDirectory, "Google Drive"), 1, extractedFlows=1)
    (data2, label2) = loadData(os.path.join(BaseDirectory, "Youtube"), 2, extractedFlows=1)
    (data3, label3) = loadData(os.path.join(BaseDirectory, "Google Doc"), 3, extractedFlows=1)
    (data4, label4) = loadData(os.path.join(BaseDirectory, "Google Search"), 4, extractedFlows=1)
    (data5, label5) = loadData(os.path.join(BaseDirectory, "Google Music"), 5, extractedFlows=1)
    ...
    test_data2 = np.concatenate((test5), axis=0)
    ...
    test_label2 = np.concatenate((testL5), axis=0)
    ...
    np.save("testData2.npy", test_data2)
    np.save("testLabel2.npy", test_label2)
